...
class StudentsPerformance():
  ...
  def ClassifierComparison(self, x, y, clf_name, clf):
    ...
    for i in range(1,d+1):
      ...
      if cross_val_score(clf, pcaX, y).mean() > best_score:
        best_pca, best_score = i, cross_val_score(clf, pcaX, y).mean()
    ...
    self.classifierScores[clf_name] = round(best_score*100,2)
    self.classifierPCA[clf_name] = best_pca
    print("%s works best with %s principal components with an average accuracy of %s"
          %(clf_name, int(best_pca), round(best_score*100,2)))
    return best_score
  ...
  def graphComparisons(self):
    ...
    plt.bar(range(len(self.classifierScores)),
           list(self.classifierScores.values()),
            tick_label = list(self.classifierScores.keys()))
  ...
  def confusionMatrix(self, X, y):
    ...
    for classifier in self.classifiers:
      ...
      ConfusionMatrixDisplay(CM,
                            display_labels = clf.classes_).plot(ax = axs[row][col])
      ...
      self.confusionMatrixMetrics[classifier] = {
  ...
  def confusionMatrix_metrics(self):
    ...
    for classifier in self.confusionMatrixMetrics:
      for metric in self.confusionMatrixMetrics[classifier]:
        print ("%s for %s: %s" %(metric, classifier,
         self.confusionMatrixMetrics[classifier][metric]))
      print ()
  ...
  def ensemble(self,X,y):
    ...
    for r in range(2, len(self.classifiers.keys())+1):
      for combination in itertools.combinations(self.classifiers.keys(),r):
        ...
        if round(accuracy_score(y_test,final_y_pred)*100,2) > bestComboScore:
          ...
          bestComboScore = round(accuracy_score(y_test,final_y_pred)*100,2)
          bestComboCM = confusion_matrix(y_test, final_y_pred)

    #Creating the final confusion matrix
    ConfusionMatrixDisplay(bestComboCM,
    ...
    plt.show()
    print("The best ensemble is", bestCombo)
